...
- The public HTTP and HTTPS listeners are network trust boundaries. Sink
  terminates managed TLS. A local TLS service terminates TLS for a passthrough
  namespace.
- When an optional TCP proxy supplies PROXY v2 metadata, it is trusted only
  from explicitly configured canonical peer CIDRs. Sink's `required-v2` mode
  rejects untrusted peers and missing, malformed, oversized, unsupported, or
  `LOCAL` headers before TLS dispatch.
...
Sink replaces visitor-supplied forwarding headers with the public host, scheme,
and observed visitor address before sending a request to the local application.
When another network layer is present, treat address metadata as trusted only
when it comes from a configured peer.
...
client constructs a fresh header and does not copy inbound TLVs. The local TLS
service must require PROXY v2 only on this listener and trust only the Sink
client peer; accepting it from an application or visitor network would allow
...
Sink for managed namespaces and at the local TLS service for passthrough
namespaces; Sink never terminates, inspects, or supplies its certificate.
HTTP-to-HTTPS redirects are application policy and are not added by this
implementation.
